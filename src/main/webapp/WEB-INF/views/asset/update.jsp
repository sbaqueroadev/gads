...
<body>
	...
	<div id="content" ng-app='assetUpdateApp'>
		<form id="updateAssetForm" ng-controller="assetUpdateCtrlr as vm"
			...
			<div class="col-md-offset-2 col-md-6">
				Asignar a:<br>
				<label for="person">Persona:</label> <select class="form-control"
					id="person" name="person" ng-model="asset.assignedAsset.person.id"
					ng-change="assignedSelected('person')">
					<option ng-repeat="person in persons"
						value="{{person.id}}">{{person.id}} - {{person.name}}</option>
				</select>
				<br>
				<label for="area">Area:</label> <select class="form-control"
					id="area" name="area" ng-model="asset.assignedAsset.area.id" 
					ng-change="assignedSelected('area')">
					<option ng-repeat="area in areas"
						value="{{area.id}}">{{area.id}} - {{area.name}}, {{area.city.name}}</option>
				</select>
			</div>
